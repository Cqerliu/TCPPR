...
class PromoterEncoder:
    ...
    def generate_kmers(self):
        ...
        if self.sequences is None:
            raise ValueError("Please set the sequences first with set_sequences_labels")
        ...
        if len(set(lengths)) != 1:
            raise ValueError("The k-mer length must be the same for all sequences")
    ...
    def train_word2vec(self):
        ...
        if self.kmers_list is None:
            raise ValueError("Please generate the k-mer sequences first")
    ...
    def get_transformer_input(self):
        ...
        if self.word_vectors is None:
            raise ValueError("Please train the Word2Vec model first")
        ...
        for kmers in self.kmers_list:
            ...
            if len(vecs) != self.sequence_length:
                raise ValueError(f"k-mer vector length mismatch：{len(vecs)}（expected：{self.sequence_length}）")
